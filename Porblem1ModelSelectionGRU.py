...
GRU_test_RMSEs =  [[[[[0 for x in batch_size] for x in nodes] for x in learning_rate] for x in layers] for x in optimizer_types]
GRU_train_RMSEs = GRU_test_RMSEs
    ...
    for n,node in enumerate(nodes): 
      for l,layer in enumerate(layers): 
        for o,optimizer in enumerate(optimizer_types):
          for i in [0,1]: #model averaging for loop
              ...
              if layer == 1:
                model = tf.keras.models.Sequential([
                  ...
                  tf.keras.layers.GRU(node, return_sequences=True),
          ...
          GRU_test_RMSEs[o][l][r][n][b] = np.average(test_RMSEs)
          GRU_train_RMSEs[o][l][r][n][b] = np.average(train_RMSEs)

          counter+=1
          print('Finished run # ', counter, ' of ', len(batch_size)*len(learning_rate)*len(nodes)*len(layers)*len(optimizer_types),
                '| This runs Test RMSE = ', GRU_test_RMSEs[o][l][r][n][b])
          
#%%
test=GRU_test_RMSEs
train=GRU_train_RMSEs
test=list(np.concatenate(test).flat)
train=list(np.concatenate(train).flat)
testdf = pd.DataFrame(test)
testdf.to_csv('GRUtestRMSEs.csv')
traindf = pd.DataFrame(train)
traindf.to_csv('GRUtrainRMSEs.csv')
# %%
