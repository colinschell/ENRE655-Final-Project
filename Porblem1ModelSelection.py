...
RNN_test_RMSEs =  [[[[[0 for x in batch_size] for x in nodes] for x in learning_rate] for x in layers] for x in optimizer_types]
RNN_train_RMSEs = RNN_test_RMSEs
print(np.shape(RNN_train_RMSEs))
    ...
    for n,node in enumerate(nodes): 
      for l,layer in enumerate(layers): 
        for o,optimizer in enumerate(optimizer_types):
          ...
          RNN_test_RMSEs[o][l][r][n][b] = np.average(test_RMSEs)
          RNN_train_RMSEs[o][l][r][n][b] = np.average(train_RMSEs)

          counter+=1
          print('Finished run # ', counter, ' of ', len(batch_size)*len(learning_rate)*len(nodes)*len(layers)*len(optimizer_types), 
                '| This runs Test RMSE = ', RNN_test_RMSEs[o][l][r][n][b])
...
#testdf.to_csv('RNNtestRMSEs.csv')
traindf = pd.DataFrame(train)
#traindf.to_csv('RNNtrainRMSEs.csv')
# %%
