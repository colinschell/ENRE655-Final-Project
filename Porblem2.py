...
#%%
dataset = np.load('CMAPPS_FD004_nonsc.npz')
...
x_train = np.reshape(X_train_pre_img,[-1,19,14])
x_test = np.reshape(X_test_pre_img,[-1,19,14])
...
# Initialize the Artificial Neural Network (ANN) and build the Graph.
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.LSTM(64, return_sequences=True),
        #tf.keras.layers.LSTM(32, return_sequences=True),# Try setting to False and see what happens
